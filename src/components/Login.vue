      ...
      <q-card v-if="!isRegister" class="login-panel col-lg-9 col-xs-10 ">
        ...
        <q-card-main class="row col-10 justify-center ">
          ...
          <div class="row col-10 justify-between">
            <q-select @change="changeLang" class="col-auto " chips color="secondary" v-model="lang" :options="langsOpts" />
    ...
    async login(e, done) {
      ...
      try {
        ...
        if (data.success === true) {
          ...
          this.remember ? this.$session.set('user', user) : this.$session.remove('user')
          // 是否登录的全局变量
          this.$router.push({
            name: 'home',
            params: {
